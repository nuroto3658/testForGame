...
async function glob(pattern, cwdOrFastGlobOptions) {
    ...
    const result = await (0, fast_glob_1.default)(pattern, opts);
    const cwd = (opts) ? opts.cwd : undefined;
    const list = result.map(path => {
        return (cwd) ? (0, path_1.join)(cwd, path) : path;
    });
...
async function saferRemove(names, cwd) {
    const baseDir = (cwd) ? (0, path_1.resolve)(cwd) : (0, path_1.resolve)('./');
    let removedNames = [];
    for (const name of asArray(names)) {
        ...
        const exists = await (0, fs_extra_1.pathExists)(fullPath);
        if (exists) {
